<script setup lang="ts">

</script>

<template>
  <nav class="py-5 fixed top-0 z-20 w-full">
    <div class="custom-container px-10">
      <ul class="inline-flex bg-gray-50 p-2 rounded-full shadow">
      <li class="flex items-center mr-8">
        <div class="border-8 border-emerald-400 rounded-full bg-slate-200 w-px h-px ml-4 mr-2 p-1.5">
          <span class="bg-slate-800 rounded-full w-1.5 h-1.5 flex"></span>
        </div>
        <h2 class="text-gray-700 text-sm">Knowleducation</h2>
      </li>
      <li class="flex items-center text-gray-400 font-sans font-bold">
        <span class="text-xs cursor-pointer mr-4">Courses</span>
        <span class="text-xs cursor-pointer">Contact Us</span>
        <span class="text-xs cursor-pointer ml-4">Lorem</span>
      </li>
      <li class="bg-emerald-400 cursor-pointer ml-8 px-6 py-2 rounded-full text-slate-50 text-xs">Contact Us</li>
    </ul>
    </div>
   
  </nav>
</template>

<style scoped>
/* nav class px-8  */
</style>